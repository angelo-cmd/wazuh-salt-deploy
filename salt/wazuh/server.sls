...
# Create a tar archive of certs
compress_wazuh_certs:
  cmd.run:
...
unpack_wazuh_certs:
  cmd.run:
    - name: tar -xf ./wazuh-certificates.tar -C /etc/filebeat/certs/ ./{{ node_name }}.pem ./{{ node_name }}-key.pem ./admin.pem ./admin-key.pem ./root-ca.pem
    - runas: root
    - require:
      - cmd: compress_wazuh_certs
      - file: deploy_wazuh_certs_dir

# Rename certificate files
rename_filebeat_cert:
  cmd.run:
    - name: mv -n /etc/filebeat/certs/{{ node_name }}.pem /etc/filebeat/certs/filebeat.pem
...
rename_filebeat_key:
  cmd.run:
    - name: mv -n /etc/filebeat/certs/{{ node_name }}-key.pem /etc/filebeat/certs/filebeat-key.pem
